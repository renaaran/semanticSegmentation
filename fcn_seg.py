...
def load_model():
    global model
    if opt.model == 50:
        model = fcn_resnet50(num_classes=FacadesDataset.NUM_LABELS,
                                   pretrained=False)
    elif opt.model == 101:
        model = fcn_resnet101(num_classes=FacadesDataset.NUM_LABELS,
                                    pretrained=False)
...
def create_dataset():
    ...
    train_dataset = FacadesDataset(opt.dataroot, root='train')
    print(f'train_dataset size={len(train_dataset)}')
    train_loader = DataLoader(train_dataset, shuffle=True, batch_size=2)
    val_dataset = FacadesDataset(opt.dataroot, root='val')
