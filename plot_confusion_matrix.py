...
def load_model(model_type, model_path, n):
    if model_type == 50:
        model = deeplabv3_resnet50(num_classes=FacadeDataset.NUM_LABELS, 
                                   pretrained=False)
    elif model_type == 101:
        model = deeplabv3_resnet101(num_classes=FacadeDataset.NUM_LABELS, 
                                    pretrained=False)
...
def evaluate_test_dataset(exp, dataset, classes, N=5):
    ...
    for i in [4]:
        ...
        for X, y in test_loader:
            ...
            i += 1
            print(i)
            _y = np.concatenate((_y, y.numpy().flatten()))
            _y_pred = np.concatenate((_y_pred, y_pred.numpy().flatten()))
            if i == 1: break
...
#    device = 'cpu'
    evaluate_test_dataset(exp, 'geo', GeoDataset.LABELS)
