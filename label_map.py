...
IDX_PATH = '/home/CAMPUS/180178991/Desenv/semanticSegmentation/datasets/facades/train'
CYC_PATH = '/media/180178991/Data/pytorch-CycleGAN-and-pix2pix/results/facades_cyclegan/test_latest/images'
DST_PATH = '/home/CAMPUS/180178991/Desenv/semanticSegmentation/datasets/facades+rec_B.New/train'
    ...
    for i in range(real.size[0]):
        for j in range(real.size[1]):
            ...
            if c not in index_map:
                ...
                raise Exception(f'Invalid label {p} at ({i},{j})')

    return index_map

if __name__ == "__main__":
    cnt = 303
    for file_name in sorted(glob.glob(os.path.join(IDX_PATH, '*.png'))):
        print(os.path.basename(file_name), cnt)
        idx = regex.findall(os.path.basename(file_name))[0]
        fake = np.array(Image.open(os.path.join(CYC_PATH, f'{idx}_fake_A.png')))
        real = Image.open(os.path.join(CYC_PATH, f'{idx}_real_B.png'))
        indexed = Image.open(file_name)
        ...
        fake_indexed.save(os.path.join(DST_PATH, f'{cnt}.png'))
        recb = Image.open(os.path.join(CYC_PATH, f'{idx}_rec_B.png'))
        recb.save(os.path.join(DST_PATH, f'{cnt}.jpg'))
        cnt += 1